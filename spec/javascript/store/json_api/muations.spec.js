...
describe("Mutations", function() {
  ...
  describe("#removeFromAll", function() {
    it("remove from assotiation", function() {
      let user = { id: 1, type: 'users', relationships: {
        issues: { data: [{ id: 1, type: 'issues' }] }
      }}
      let issue = { id: 1, type: 'issues'}

      let store = { issues: [issue], users: [user] }

      Mutations.removeFromAll(store, {
        child: issue, parent_type: 'users', parent_relationship_name: 'issues'
      })

      expect(store.users[0].relationships.issues.data).to.eql([])
    });
  })
  ...
  describe("#setAssociation", function() {
    it("add to association", function() {
      ...
      Mutations.setAssociation(state, {
        parent: user, child: issue, relationship_name: 'user'
      })
