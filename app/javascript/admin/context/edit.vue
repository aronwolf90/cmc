<template lang='pug'>
  b-form(@submit="submit")
    b-form-group(
      id="input-time-zone-label",
      label="Time zone",
      label-for="input-time-zone"
    )
      b-form-select(
        id="input-time-zone",
        v-model="form.attributes['time-zone']",
        :options="timeZoneOptions",
        :state="errorStatus('attributes/time-zone')"
      )
        <option value=""></option>
        ...
        <option selected="selected" value="Berlin">(GMT+01:00) Berlin</option>
      ...
      b-form-invalid-feedback(
        ...
        | {{ error.detail }}
    b-form-checkbox(
      id="checkbox-global-board"
      v-model="form.attributes['global-board']"
    ) Global board
    br
    b-button(type="submit", variant="success") Update
</template>
...
export default {
  ...
  created () {
    this.$store.dispatch('getContext').then(response => {
      this.form.attributes['time-zone'] = response.data.attributes['time-zone']
      this.form.attributes['global-board'] = response.data.attributes['global-board']
    })
  },
  computed: {
    context () {
  ...
  methods: {
    submit (event) {
      ...
      this.$store.dispatch('updateContext', {
        ...
        location.replace('/administration/admin/context')
      }).catch(({ status, data }) => {
