      ...
      class AssociationManager
        ...
        def association_instance
          ...
          if attributes_hash = nested_attribute_hash
            factory.build_class.where(attributes_hash.attributes(FindAttributes)).first ||
...
FactoryBot.factories.each do |factory|
  ...
  factory.human_names.each do |human_name|
    ...
    attribute_names_for_model.each do |attribute_name|
      ...
      Given(/^an? #{human_name} exists with an? #{human_column_name} of "([^"]*)"$/) do |value|
      ...
      Given(/^(\d+) #{human_name.pluralize} exist with an? #{human_column_name} of "([^"]*)"$/) do |count, value|
